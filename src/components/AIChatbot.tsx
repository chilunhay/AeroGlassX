...
export default function AIChatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Xin chào! Tôi là Aero, trợ lý ảo của kính thực tế không gian AeroGlass X. Tôi có thể tư vấn giá bán (từ 37.490.000 đ), trọng lượng siêu nhẹ (75g), màn hình 8K và ưu đãi giảm giá 20% khi đặt trước cho bạn!",
    },
  ]);
  ...
  const quickReplies = [
    { label: "Thông Số & Thấu Kính", text: "Thông số thấu kính và màn hình hiển thị của AeroGlass X là gì?" },
    { label: "Giá Cả Phiên Bản", text: "Các mẫu kính Tiêu chuẩn, Pro và DevKit giá bao nhiêu?" },
    { label: "Hỗ Trợ Kính Cận", text: "Kính có hỗ trợ tròng cận ZEISS hay không?" },
  ];
  ...
  const getOfflineResponse = (query: string): string => {
    ...
    if (text.includes("thong so") || text.includes("specs") || text.includes("optic") || text.includes("display") || text.includes("8k") || text.includes("man hinh") || text.includes("hien thi") || text.includes("phan giai") || text.includes("quat")) {
    ...
    if (text.includes("gia") || text.includes("bao nhieu") || text.includes("cost") || text.includes("tien") || text.includes("standard") || text.includes("pro") || text.includes("devkit") || text.includes("mua")) {
    ...
    if (text.includes("can") || text.includes("vien") || text.includes("zeiss") || text.includes("nam cham") || text.includes("mat") || text.includes("trong")) {
    ...
    if (text.includes("pin") || text.includes("battery") || text.includes("tieng") || text.includes("gio") || text.includes("dung luong") || text.includes("sac")) {
    ...
    if (text.includes("ship") || text.includes("giao") || text.includes("nhan") || text.includes("rollout") || text.includes("huy") || text.includes("coc") || text.includes("hoan")) {
    ...
    if (text.includes("helicorp") || text.includes("cong ty") || text.includes("tap doan") || text.includes("ai lam")) {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      // POST payload to Next API route
      const response = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          messages: [...messages.filter(m => !m.isFallback), userMessage].map(m => ({
            role: m.role,
            content: m.content
          })),
        }),
      ...
      // Run smart offline response fallback
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    <span className={`w-1.5 h-1.5 rounded-full ${isFallbackMode ? "bg-amber-500 animate-pulse" : "bg-emerald-500"}`} />
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-muted/30 border border-border/40 text-foreground px-4 py-3 rounded-2xl rounded-tl-none flex flex-col gap-1.5">
                    ...
                    <span className="text-[8px] font-bold text-muted-foreground/60 uppercase tracking-widest">Aero đang gõ...</span>
                  </div>
                </div>
              )}
              <div ref={chatEndRef} />
